...
Course * create_course(char *code, int credit, char *grade) {
    ...
    course->grade = calloc(strlen(grade) + 1, sizeof(char));  // Allocate memory for grade
...
double compute_gpa(Student *student) {
    ...
    for(int i = 0; i < student->course_count; i++) {
        ...
        double final_points = get_points(course->grade);
        
        // Accumulate total points and credits
        total_points += final_points * course->credit;
...
void introduce(Student *student) {
    ...
    printf("%s %.2f", student->name, compute_gpa(student));
}
